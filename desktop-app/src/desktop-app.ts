  ...
  constructor() {
    ...
    autoUpdater.on('update-available', (event, releaseNotes, releaseName) => {
      ...
      const dialogOpts = {
        ...
        title: 'New Version',
        message: 'An update for the FastFeelessBitcoin-Wallet is available!',
    ...
    autoUpdater.on('error', message => {
      ...
      mainWindow.setTitle(`FastFeelessBitcoin-Wallet - ${autoUpdater.currentVersion}`); // reset title
      showUpdateErrors = false; // disable errors
      const dialogOpts = {
        type: 'error',
        buttons: ['OK'],
        title: 'Update Error',
        message: 'Something went wrong while downloading FastFeelessBitcoin-Wallet.',
...
function createWindow () {
  ...
  mainWindow.webContents.on('did-finish-load', function () {
    mainWindow.setTitle(`FastFeelessBitcoin-Wallet - ${autoUpdater.currentVersion}`);
  });
...
function sendStatusToWindow(progressObj) {
  ...
  mainWindow.setTitle(`FastFeelessBitcoin-Wallet - ${autoUpdater.currentVersion} - Downloading Update: ${Math.round(progressObj.percent)} %`);
}
...
if (!appLock) {
  ...
  // Refocus the window if the user attempts to open FastFeelessBitcoin-Wallet while it is already open
...
function getApplicationMenu() {
  const template: any = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'FastFeelessBitcoin-Wallet Help Docs',
  ...
  if (process.platform === 'darwin') {
    template.unshift({
      label: 'FastFeelessBitcoin-Wallet',
      submenu: [
        {role: 'about'},
        {type: 'separator'},
        {
