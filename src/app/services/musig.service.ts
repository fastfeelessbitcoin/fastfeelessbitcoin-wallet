...
export class MusigService {
  ...
  aggregate(storedAccounts, runWithPubkeys = null) {
    ...
    for (let address of addresses) {
      ...
      if (!address.startsWith('fbtc_') && !address.startsWith('fbtc_') && !address.startsWith('nano_')) {
        throw new Error('FastFeelessBitcoin addresses must start with fbtc_');
      }
      address = address.split('_', 2)[1];
      try {
        const bytes = base32.decode(address);
        if (bytes.length !== 37) {
          throw new Error('Wrong FastFeelessBitcoin address length');
        }
        ...
        if (!this.util.array.equalArrays(bytes.subarray(32), checksum_)) {
          throw new Error('Invalid FastFeelessBitcoin address checksum');
        }
        pubkeys.push(pubkey);
      } catch (err_) {
          console.error(err_.toString());
          throw new Error('Invalid FastFeelessBitcoin address (bad character?)');
      }
    }
    ...
    const fullAddressFinal = 'fbtc_' + base32.encode(fullAddress);
