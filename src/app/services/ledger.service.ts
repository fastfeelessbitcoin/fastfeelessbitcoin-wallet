  ...
  async loadLedger(hideNotifications = false) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
          ...
          if (err.name !== 'TransportOpenUserCancelled') {
            ...
            if (!hideNotifications) {
              this.notifications.sendWarning(`Ledger connection failed. Make sure your Ledger is unlocked.  Restart the FastFeelessBitcoin app on your Ledger if the error persists`);
            }
          }
          this.resetLedger();
          resolve(false);
      ...
      setTimeout(() => {
        ...
        if (!hideNotifications) {
          this.notifications.sendWarning(`Unable to connect to the Ledger device.  Make sure it is unlocked and the FastFeelessBitcoin application is open`);
        }
        resolved = true;
        return resolve(false);
      }, 10000);
      ...
      try {
        ...
        this.ledger.status = LedgerStatus.NOT_CONNECTED;
        this.ledgerStatus$.next({ status: this.ledger.status, statusText: `Unable to load FastFeelessBitcoin App configuration: ${err.message || err}` });
        if (err.statusText === 'HALTED') {
          this.resetLedger();
        }
        if (!hideNotifications && !resolved) {
          this.notifications.sendWarning(`Unable to connect to the Ledger device.  Make sure your Ledger is unlocked.  Restart the FastFeelessBitcoin app on your Ledger if the error persists`);
        }
        resolved = true;
        return resolve(false);
      }
      ...
      try {
        ...
        if (err.statusCode === STATUS_CODES.SECURITY_STATUS_NOT_SATISFIED) {
          ...
          if (!hideNotifications) {
            this.notifications.sendWarning(`Ledger device locked.  Unlock and open the FastFeelessBitcoin application`);
          }
        }
      }
