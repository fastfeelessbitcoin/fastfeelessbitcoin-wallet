...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  remoteVisible = false;
  blockTypes: string[] = ['Send FastFeelessBitcoin', 'Change Representative'];
  ...
  async receiveReceivableBlock(receivableBlock) {
    ...
    if (createdReceiveBlockHash) {
      ...
      this.notifications.sendSuccess(`Successfully received FastFeelessBitcoin!`, { identifier: 'success-receive' });
  ...
  async generateSend() {
    ...
    if (!this.validateAmount()) return this.notifications.sendWarning(`Invalid FBTC Amount`);
    ...
    if (from.balanceBN.minus(rawAmount).lessThan(0)) return this.notifications.sendError(`From account does not have enough FBTC`);
    ...
    const blockData = {
      account: this.accountID.replace('nano_', 'fbtc_').toLowerCase(),
    ...
    const blockDataPrevious = {
      account: jsonBlock.account.replace('nano_', 'fbtc_').toLowerCase(),
