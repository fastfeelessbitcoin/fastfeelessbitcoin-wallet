...
export class ManageWalletComponent implements OnInit {
  ...
  async exportToFile() {
    ...
    const fileName = `FastFeelessBitcoin-Wallet-Wallet.json`;
  ...
  async exportToCsv() {
    ...
    if (this.invalidCsvCount) {
      if (this.beyondCsvLimit) {
        return this.notifications.sendWarning(`To export transactions above the limit, please use a custom FastFeelessBitcoin-Wallet server`);
      } else {
