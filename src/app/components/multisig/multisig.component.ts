...
export class MultisigComponent implements OnInit {
  ...
  addAccount() {
    if (this.accountAddStatus !== 1) {
      ...
      this.notificationService.sendWarning('Invalid FastFeelessBitcoin address!', {identifier: 'account-invalid'});
      return;
    }
    if (this.storedAccounts.includes(this.accountAdd.replace('nano_', 'fbtc_').toLocaleLowerCase())) {
    ...
    this.storedAccounts.push(this.accountAdd.replace('nano_', 'fbtc_').toLocaleLowerCase());
    this.accountAdd = '';
  ...
  navigateAccount() {
    if (this.validateMultisig()) {
      ...
      this.notificationService.sendWarning('Invalid FastFeelessBitcoin account!');
    }
  }
